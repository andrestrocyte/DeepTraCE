...
def save_refined_model(refined_model, save_path):
    """
    Saves the refined_model to the specified path as a .npy file.

    Parameters:
        refined_model (numpy.ndarray): The array to be saved.
        save_path (str): The file path (including file name) to save the array.
    """
    try:
        np.save(save_path, refined_model)
        print(f"refined_model successfully saved to: {save_path}.npy")
    except Exception as e:
        print(f"Error saving refined_model: {e}")


def load_refined_model(load_path):
    """
    Loads the refined_model from the specified .npy file path.

    Parameters:
        load_path (str): The file path (including file name) to load the array from.

    Returns:
        numpy.ndarray: The loaded refined_model array.
    """
    try:
        refined_model = np.load(load_path)
        print(f"refined_model successfully loaded from: {load_path}")
        return refined_model
    except Exception as e:
        print(f"Error loading refined_model: {e}")
        return None
